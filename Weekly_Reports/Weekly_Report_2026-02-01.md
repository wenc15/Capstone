...
### 3️⃣ 白名单可用性升级（exe 识别改进版）###

**目标：让白名单在 Windows 上对常见启动方式都能“正确识别并放行”（尤其是快捷方式、游戏平台、自动更新器这种场景）
（中文hint：解决“选了 Discord/Steam 但识别成 Update.exe / 或完全识别不了”的问题）**

**增强：应用识别 = 不只看 ProcessName**

- 后端在判定“是否允许”时，不仅使用进程名（xxx.exe），而是优先获取并匹配：
- FullPath（进程完整路径）
- Parent process（父进程，用于处理 Steam/Discord 启动链）

**Command line（部分游戏/启动器只在参数里有线索）**

- 允许策略：如果用户白名单里包含 steam.exe，但实际启动了 game.exe（由 steam 拉起），应能被识别为 允许
- （中文hint：用路径/父子进程链解决“启动器/更新器/代理进程”）
